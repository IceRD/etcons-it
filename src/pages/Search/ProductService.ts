import { IProduct, FilterFields } from './types'
...
const ProductService = {
	...
	getProductsMini(): IProduct[] {
		return this.getProductsData().slice(0, 5)
	},

	getProductsSmall(): IProduct[] {
		...
		return this.getProductsData().filter((product) => {
			for (let i = 0; i < filters.length; i++) {
				...
				if (
					...
					String(product[key]).toLocaleLowerCase().includes(search.toLocaleLowerCase())
				) {
					return true
				}
			}
